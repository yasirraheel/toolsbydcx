...
(function(){
  'use strict';
  var BF_DEFAULT_SERVER = 'http://localhost:5000';
...
const BUNNYFLOW_INJECT_URL = 'http://localhost:5000/api/extension/inject-cookies';
...
// flowbybunny.com tab so no other page can trigger it.
chrome.runtime.onMessage.addListener(function (msg, sender, sendResponse) {
  if (!msg || msg.type !== 'SITE_LOGOUT') return false;
  var ok = false;
  try {
    ok = !!(sender && sender.tab && typeof sender.tab.url === 'string' &&
      (/^https?:\/\/(www\.)?(flowbydcx\.com|localhost|127\.0\.0\.1)/.test(sender.tab.url)));
  } catch (e) {}
  if (ok) {
    Promise.resolve()
